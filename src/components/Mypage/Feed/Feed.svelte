<script lang="ts">
    ...
    const handleDeleteMyFeed = (feedId: number, index: number) => {

        popup("피드를 삭제하시겠습니까?", 3, async (data: boolean) => {
            ...
            if(response.statusCode == 1) {
                myFeedList = myFeedList.filter((_, i) => i !== index);
            }
...
<MyPage>
    <ul class="my-feed">
            ...
            {#each myFeedList as item, index}
                <li class="my-feed__list">
                    ...
                    <div class="my-feed__list__body">
                        ...
                        <button 
                            class="small red my-feed__list__body__btn"
                            on:click={()=>{handleDeleteMyFeed(item.id, index)}}
                        >
                            삭제
                        </button>
                    </div>
        ...
        {:else}
            <li class="my-feed__list">
                <p class="my-feed__list__none">작성된 피드가 없습니다.</p>
            </li>
        {/if}
    </ul>
