<script lang="ts">
    ...
    const getExchanges = async () => {
        ...
        if (urlQuery !== previousUrl) {
            ...
            if (exchangeData || response.statusCode === 1) {
                ...
                chartSortOrder.set(OrderTypeModel.DESC);
    ...
    const hasAllKeysAddedD = (value) => {
        return Object.values(value).every(
            (set) => set && typeof set === "object" && "tp" in set,
        );
    };
    ...
    const sortTickers = (e: any, target: string) => {
        ...
        if (sortTarget == target) {
            sortOrder =
                sortOrder == OrderType.ASC ? OrderType.DESC : OrderType.ASC;
        } else {
    ...
    const resetOrderButton = (reset = false) => {
        ...
        if (
            reset == true ||
            (get(chartSortOrder) == OrderType.DESC &&
                get(chartSortTarget) == "atp24h")
        ) {
    ...
    init();
    $: filteredTickers = Object.values(tickers).filter(
        (ticker) => ticker.s?.toUpperCase() === symbolInfo.s.toUpperCase(),
    );
